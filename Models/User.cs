...
{ // user update
    ...
    class SuperAdmin : User // create a class for super admin that inherits from User
    {
        ...
        private string GenerateEmail(string name, string role) // method to generate email based on name and role
        {
            return $"{name.ToLower().Replace(" ", ".")}@{role.ToLower()}.healthsystem.com"; // format email as name.role.com
        }




        public string AddAdmin(string name, string password) // method to add an admin
        {
            string autoGeneratedEmail = GenerateEmail(name, "admin"); // generate email for the new admin
            Admins newAdmin = new Admins(name, autoGeneratedEmail, password); // create a new admin with the next index
            this.UsersList.Add(newAdmin); // add the new admin to the list of users
            return autoGeneratedEmail;
        }

        public string AddDoctor(string name,  string password, string Spelcialization) // method to add a doctor
        {
            string autoGeneratedEmail = GenerateEmail(name, "doctor"); // generate email for the new doctor
            Doctor newDoctor = new Doctor(0, name, autoGeneratedEmail, password, Spelcialization); // create a new doctor with the next index
            this.UsersList.Add(newDoctor); // add the new doctor to the list of users
            return autoGeneratedEmail; // return the generated email
        }
